...
const PromoRenderer = {
    ...
    renderBrands() {
        ...
        const brandItem = (brand) => {
            const brandName = brand.nombre.toLowerCase().replace(/\s+/g, '');
            const imgSrc = `imagenes/${brandName}.svg?v=1`;
            return `
...
const AdminPanel = {
        ...
        const panelHtml = `
            <div id="admin-panel" class="dk-modal-container fixed top-4 right-4 z-50 bg-white rounded-2xl shadow-2xl w-80 overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                    ...
                    <button onclick="GitHubPublisher.showTokenModal()" class="w-full flex items-center gap-3 bg-blue-500 hover:bg-blue-600 text-white p-3.5 rounded-xl text-sm font-semibold transition-all group">
                        ...
                        <span>⚙️ Configurar GitHub</span>
                    </button>
                    <button id="publish-btn" onclick="GitHubPublisher.publish()" class="w-full flex items-center gap-3 bg-green-500 hover:bg-green-600 text-white p-3.5 rounded-xl text-sm font-semibold transition-all group">
                        <div class="w-9 h-9 bg-white/20 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
    ...
    saveProduct(id) {
        ...
        if (GitHubPublisher.getToken()) {
            alert(id ? 'Producto actualizado. Publicando...' : 'Producto agregado. Publicando...');
            GitHubPublisher.publish();
        } else {
            alert((id ? 'Producto actualizado' : 'Producto agregado') + '. Necesitas configurar el token de GitHub para publicar.');
        }
    },
    ...
    deleteProduct(id) {
        if (confirm('¿Eliminar este producto?')) {
            ...
            App.init();
            if (GitHubPublisher.getToken()) {
                alert('Producto eliminado. Publicando...');
                GitHubPublisher.publish();
            } else {
                alert('Producto eliminado. Necesitas configurar el token de GitHub para publicar.');
            }
        }
    },
